...
public abstract class Query<V> extends IOTask<V> {
    ...
    private QueryProperties parameters = new QueryProperties(true);
    private QueryProperties requestHeaders = new QueryProperties(false);
    private QueryProperties responseHeaders = new QueryProperties(false);
    ...
    public URL getLocation() {
        ...
        for (String key : parameters.keySet()) {
            for (int index = 0; index < parameters.size(key); index++) {
                try {
    ...
    protected Object execute(Method method, Object value) throws QueryException {
        ...
        try {
            ...
            for (String key : requestHeaders.keySet()) {
                for (int i = 0, n = requestHeaders.size(key); i < n; i++) {
                    if (i == 0) {
